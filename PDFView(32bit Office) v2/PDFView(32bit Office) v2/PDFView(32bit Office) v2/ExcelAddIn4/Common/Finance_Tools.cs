...
namespace ExcelAddIn4
{
    public partial class Finance_Tools
    {
        ...
        /// <summary>
        /// for transaction search by prefix
        /// </summary>
        /// <returns></returns>
        public int? ProcessMaxNumber(string prefix)
        {
            var maxnum = (from FT_sett in db.rsTemplateTransactions
                          where FT_sett.Prefix == prefix
        ...
        public DataGridView IniGrd()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    combox4.HeaderText = "Post Provisional";
                    combox4.Name = "PostProvisional";
                    combox4.DataPropertyName = "PostProvisional";
                    combox4.SortMode = DataGridViewColumnSortMode.NotSortable;
                    combox4.DataSource = IniPP();
                    combox4.DisplayMember = "name";
                    combox4.ValueMember = "value";
                    d.Columns.Add(combox4);
                    d.Columns.Add("LineIndicator", "Line Indicator");
                    d.Columns["LineIndicator"].DataPropertyName = "LineIndicator";
                ...
                d.Columns["PostProvisional"].ToolTipText = "Post Provisional";
                return d;
            }
            catch (Exception ex)
            {
        ...
        public string GetXMLScript(List<Line> list)
        {
            try
            {
                ...
                de.MethodContext.LedgerPostingParamrters.PostProvisional = string.IsNullOrEmpty(SessionInfo.UserInfo.PostProvisional) ? "N" : SessionInfo.UserInfo.PostProvisional;
        ...
        public void GetInvoiceInfo(ref string prefix, ref string invName, ref int? i)
        {
            if (SessionInfo.UserInfo.UseSequenceNumbering == "1")
            {
                try
                {
                    ...
                    var maxnum = ProcessMaxNumber(prefix);
                    string IDvalue = string.Empty;
                    if (SessionInfo.UserInfo.Dictionary.dict.Count != 0 && SessionInfo.UserInfo.Dictionary.dict.ContainsKey(SessionInfo.UserInfo.CachePath))
                    {
                        IDvalue = SessionInfo.UserInfo.Dictionary.dict[SessionInfo.UserInfo.CachePath];
        ...
        public DataGridView IniXMLFormGrd()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                d.Columns.Add("PostProvisional", "PostProvisional");
                d.Columns["PostProvisional"].DataPropertyName = "PostProvisional";
                d.Columns["PostProvisional"].Visible = false;
            }
            else
            {
                d.Columns.Add("LineIndicator", "Line Indicator");
